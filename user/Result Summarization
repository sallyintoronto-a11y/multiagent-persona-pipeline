# === Summaries to JSON only + Download to local (USER-PROMPT VERSION) ===
...
# ---- CONFIG ----
OUT_DIR = "./outputs_userprompt"   # Root directory created by run_batch (user-prompt version)
ROUND_DIGITS = 3
...
def summarize_pair(pair_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pair_overall = round(axis_avg["factuality"] + axis_avg["norm_appropriateness"] + axis_avg["bias_presence"], ROUND_DIGITS)
...
# ---- Build summaries from outputs_userprompt/by_image/*.json ----
...
# ---- Save JSONs (user) ----
summary_dir = os.path.join(OUT_DIR, "summary_user")
Path(summary_dir).mkdir(parents=True, exist_ok=True)

master_json_path = os.path.join(summary_dir, "summary_master_user.json")
with open(master_json_path, "w", encoding="utf-8") as f:
    json.dump(master, f, ensure_ascii=False, indent=2)

flat_json_path = os.path.join(summary_dir, "summary_pairs_user.json")  # CSV 대체
